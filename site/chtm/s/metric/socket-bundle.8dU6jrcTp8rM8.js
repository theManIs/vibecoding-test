...
function nanoid(size) {
    alphabet = '_zyxwvutsrqponmlkjihgfedcba9876543210ZYXWVUTSRQPONMLKJIHGFEDCBA';
    size = size || 21;
    var id = '';
    while (size--) {
...
class SocketStore {
    ...
    on(event, cb, delay = 0) {
        ...
        const disposer = () => {
            ...
            disposed = true;
            this.listeners = this.listeners.filter(listener => listener.disposer !== disposer);
            if (this._io) {
    ...
    subscribeToSocket(socketId, onChange, delay = 0) {
        ...
        if (!this.socketSubscriptions[socketId]) {
            this.socketSubscriptions[socketId] = new SocketSubscription(this, socketId, () => {
    ...
    subscribeToTyping(typingData) {
        ...
        if (!this.typingSubscriptions[typingData.id]) {
            this.typingSubscriptions[typingData.id] = new TypingSubscription(this, typingData, () => {
    ...
    subscribeToData(id) {
        ...
        if (!this.dataSocketSubscriptions[id]) {
            this.dataSocketSubscriptions[id] = new DataSocketSubscription(this, id, () => {
                delete this.dataSocketSubscriptions[id];
            });
        }
        return this.dataSocketSubscriptions[id];
    }
}
class TypingSubscription {
    ...
    open() {
        this.disposers.push(this.socketStore.onConnect(() => {
            this.socketStore.emit('typing/subscribe', this.typingData);
        }));
        this.disposers.push(() => this.socketStore.emit('typing/unsubscribe', this.typingData));
        this.disposers.push(this.socketStore.on('typing/update/' + this.typingData.id, value => {
            if (value.uid !== this.typingData.uid) {
                this._onTypingChange(value);
            }
        }));
...
class SocketSubscription {
    ...
    open() {
        this.disposers.push(this.socketStore.onConnect(() => {
            this.socketStore.emit('socket/subscribe', { socketId: this.socketId }, data => {
                if (this.lastUpdatedValue &&
                    data &&
        ...
        this.disposers.push(() => this.socketStore.emit('socket/unsubscribe', { socketId: this.socketId }));
        this.disposers.push(this.socketStore.on('socket/' + this.socketId + '/updated', data => {
            this.lastUpdatedValue = data;
            this._emitChanged();
        }));
    }
...
class DataSocketSubscription {
    ...
    open() {
        ...
        this.disposers.push(() => this.socketStore.emit('dataSocket/unsubscribe', { id: this.id }));
        this.disposers.push(this.socketStore.on('dataSocket/data/' + this.id, data => {
    ...
    writeToSocket(data) {
        this.socketStore.emit('dataSocket/write', { ...data, id: this.id });
    }
}

window.clrtSocketId = "clrt-undefined:373542:e746087f92d92f6bb903f00ff8156ed0e33bfad6";
window.clrtUserSocketId = null;
